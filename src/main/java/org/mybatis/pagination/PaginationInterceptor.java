...
public class PaginationInterceptor implements Interceptor, Serializable {
    ...
    private static String sortAndFilterSql(String sql, PagingCriteria pagingCriteria) {
        ...
        if (sortFields != null && !sortFields.isEmpty()) {
            ...
            for (SortField sortField : sortFields) {
                field_order.add(sortField.getField() + " " + sortField.getDirection().getDirection());
            }
            boolean order = SqlHelper.hasOrder(sql);
            return String.format(order ? "%s, %s" : "%s order by %s", sql, Joiner.on(",").join(field_order));
        }
        return sql;
    }
    ...
    private void processIntercept(final Object[] queryArgs) {
        ...
        final PagingCriteria pageRequest = interceptor
                ...
                : new PagingCriteria(MAPPED_STATEMENT_INDEX
                , PagingCriteria.DEFAULT_SIZE
                , PagingCriteria.DEFAULT_SIZE, null);
